...
```
* could not get the test link provided by insight to find my code. I got the error: 
	* (stderr): python: can't open file './src/H1B_visa_stats.py': [Errno 2] No such file or directory
* If I run that directory through my run.sh file, it works. I also pass the tests.
```
...
```
The second test I added was made to test whether it would count the uncertified instances.

* I added a couple more entried to the current test file, then checked it against my own written answers. 
* The test seemed to work well with the test provided and my own test
```
